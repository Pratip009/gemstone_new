...
export default function CartPage() {
  ...
  if (loading) return <div className="text-neutral-500 py-20 text-center">Loading cart…</div>;
          ...
          {items.map((item) => (
            <div key={item.product._id} className="card p-4 flex gap-4 items-center">
              ...
              <div className="flex items-center border border-neutral-700 rounded">
                <button onClick={() => updateQty(item.product._id, item.quantity - 1)} className="px-2.5 py-1 text-neutral-400 hover:text-white">−</button>
              ...
              <button onClick={() => remove(item.product._id)} className="text-neutral-600 hover:text-red-400 text-lg ml-2">×</button>
            </div>
          ))}
        </div>
